...
bus=EventBus(); turns=TurnManager(); orchestrator=Orchestrator(load_agents(),bus,turns); voice=VoiceService()
class SpeakRequest(BaseModel): text:str; agent:str
class WSMgr:
...
ws=WSMgr(); bus.subscribe(ws.send)
...
@app.get("/api/agents")
async def agents(): return [{"id":a.id,"name":a.name,"role":a.role,"personality":a.personality,"voice":a.voice} for a in orchestrator.agents.values()]
@app.get("/api/health")
async def health(): return {"ok":True,"turn_locked":turns.locked(),"speaker":turns.current_speaker}
...
async def speak(payload:SpeakRequest):
 a=orchestrator.agents.get(payload.agent)
 if not a: raise HTTPException(404,"Agent fehlt")
 async with turns.speaking_turn(a.id): audio=await voice.speech(payload.text,a.voice)
...
async def websocket(w:WebSocket):
 ...
 except WebSocketDisconnect: ws.remove(w)
